...
function removeFirstOccurrences(str, value) {
  const extraValue = str.substring(
    str.indexOf(value),
    value.length + str.indexOf(value)
  );
  return str.indexOf(value) !== -1 ? str.replace(extraValue, '') : str;
}
...
function removeLastOccurrences(str, value) {
  ...
  //   value.length + str.lastIndexOf(value)
  // );
  // return str.lastIndexOf(value) !== -1 ? str.replace(extraValue, '') : str;

  const startIndex = str.lastIndexOf(value);
  const lastindex = value.length + str.lastIndexOf(value);

  if (startIndex !== -1) {
    const newStr = str.slice(0, startIndex) + str.slice(lastindex);
    return newStr;
  }
  return str;
}
...
function sumOfCodes(str) {
  if (str) {
    const arr = str.split('');
    return arr.reduce((sum, letter) => sum + letter.charCodeAt(), 0);
  }
...
function orderAlphabetically(str) {
  const arrFromCharsCode = str
    .split('')
    .map((letter) => letter.charCodeAt())
    .sort((a, b) => a - b);
  return String.fromCharCode(...arrFromCharsCode);
}
...
function countVowels(str) {
  ...
  str.split('').forEach((letter) => {
    vowels.forEach((vowel) => {
      if (letter.toLocaleLowerCase() === vowel) amountOfVowels += 1;
    });
  });
...
function isPalindrome(str) {
  ...
  return matchingLetters === arr.length;
}
  ...
  return str
    ...
    .map((l) => {
      ...
      if (alphabet.includes(l)) {
        letterInRot13 = rot13[alphabet.indexOf(l)];
      } else {
        letterInRot13 = l;
      }
      return letterInRot13;
    })
    .join('');
}
